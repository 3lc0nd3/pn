   ...
   var dd = function(element, options)
   {
	...
	var manageSelection = function (obj) {
		...
		if($("#"+childid+ " a."+styles.selected).length==1) { //check if there is any selected
			...
			//alrt("oldSelectedValue "+oldSelectedValue);
		};
		if(!ddList) {
	...
	var setValue = function () {
		...
		//alrt(getElement(elementid).selectedIndex);
		var sIndex = getElement(elementid).selectedIndex;
		$this.ddProp["selectedIndex"]= sIndex;
		//alrt("selectedIndex "+ $this.ddProp["selectedIndex"] + " sIndex "+sIndex);
	};
	...
	this.set = function(prop, val, isLocal) {
		...
		//alrt("get " + prop + " : "+$this.ddProp[prop]);
	...
	this.get = function(prop, forceRefresh) {
		...
		if(prop!=undefined && forceRefresh==undefined) {
			//alrt("c2 : " +$this.ddProp[prop]);
			return ($this.ddProp[prop]!=undefined) ? $this.ddProp[prop] : null;
		};
		if(prop!=undefined && forceRefresh!=undefined) {
			//alrt("c3 : " +getElement(elementid)[prop]);
	...
	this.disabled = function(dis, isLocal) {
		...
		//alrt(getElement(elementid).disabled);
		var id = getPostID("postID");
		if(dis==true) {
			$("#"+id).css("opacity", styles.disabled);
			$this.close();
